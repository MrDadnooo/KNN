...
def create_split_map(data_point : dataset.DataPoint) -> List[Sentence]:
    sentences = []
    for text in full_cz_from_regions(data_point):
        sentences.extend(nltk.sent_tokenize(text, language='czech'))

    text_array = [(line.text.split(), line) for line in data_point.text_lines if line.text is not None]
    ...
    for sentence in sentences:
        ...
        for word in words:
            if word == new_text_array[0][0]:
                ...
                new_text_array.remove(new_text_array[0])
        rows = set(rows)
        _map.append(Sentence(sentence, rows))
